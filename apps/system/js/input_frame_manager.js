...
(function(exports) {
  ...
  var InputFrameManager = function(keyboardManager) {
    ...
    // The set of running input windows.
    // This is a map from keyboard manifestURL to an object, which in turn maps
    // from the path (without hash) to an input window.
    // 'keyboard.gaiamobile.org/manifest.webapp' : {
    //   '/index.html': anInputWindow
    // }
    this._inputWindows = {};
  ...
  InputFrameManager.prototype.handleEvent = function ifm_handleEvent(evt) {
    ...
    switch (evt.type){
      ...
      case 'input-appclosed':
        evt.detail._setAsActiveInput(false);
        if (!this._currentWindow){
